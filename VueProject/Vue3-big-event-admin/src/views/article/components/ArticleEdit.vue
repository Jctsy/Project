...
const open = async (row) => {
    ...
    if (row.id) {
        // 需要基于 row.id 发送请求 获取对应的详细数据 进行回显
       ...
       formModel.value = res.data.data
        ...
        formModel.value = {...defaultForm} //基于默认的数据 重置 form数据
        // 这里重置了表单的数据 但是图片上传img地址 富文本编辑器内容 =>需手动重置
        imgUrl.value = ''    
  ...
  >
    ...
    <el-form :model="formModel" ref="formRef" label-width="100px">
      ...
      <el-form-item label="文章封面" prop="cover_img">
        <!-- 此处需要关闭 element-plus 的自动上传 不需要配置 action 的参数
            值需要做前端的本地预览图片即可 无需在提交前上传图片
